...
- The Worker advances exactly five steps per batch, preserving the current
  batch granularity. It schedules the next batch with a yielding Worker timer;
  it must not spin in a blocking `while` loop.
...
or clears pending requests, and calls `terminate()` exactly once. This must be
safe under React 19 StrictMode's mount → cleanup → remount development cycle.

Additional lifecycle requirements:

1. Each `run` or `reset` increments the generation before posting its command.
2. Stale `ready`, `frame`, `paused` and `error` events cannot mutate React
   state or acknowledge a current frame request.
3. Repeated `pause` and repeated cleanup are no-ops.
...
5. Unmount leaves no timer, rAF, message listener or Worker alive.
...
| WK-5 | Lifecycle | Run starts fresh, pause stops after its current batch, reset returns step 0 paused, and each rebuild clears force and spectral state. |
| WK-6 | Stale-event guard | A fake Worker proves old-generation ready/frame/error events cannot mutate current client state or release its pending request. |
...
1. `npm run test:fast` — all existing 29 tests plus WK-1…WK-8 green.
...
5. Production-browser smoke: run a cylinder, reach at least 10,000 steps, open
   ...
   Cd/Cl and a coherent final step, reset to step 0, run again, then navigate or
   reload without an orphan Worker or console error.
...
3. WK-1…WK-8, all prior fast tests and the production build pass on Node 20 and
